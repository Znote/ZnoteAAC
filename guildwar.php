...
if (!empty($_GET['warid'])) {
	...
	if ($config['ServerEngine'] == 'TFS_02' || $config['ServerEngine'] == 'TFS_10') $war = get_guild_war($warid);
	...
	if ($war != false) {
		...
		if ($config['ServerEngine'] == 'TFS_02' || $config['ServerEngine'] == 'TFS_10') $kills = get_war_kills($warid);
		...
		<ul class="war_list">
			...
			<?php if ($config['ServerEngine'] == 'TFS_02' || $config['ServerEngine'] == 'TFS_10') { ?>
			<li>
...
<a href=\"characterprofile.php?name=" . urlencode($death['name']) . "\"><b>".$death['name']."</b></a> ";
	...
	foreach ($wardata as $wars) {
		if ($config['ServerEngine'] == 'TFS_02' || $config['ServerEngine'] == 'TFS_10') $killsdata[$wars['id']] = get_war_kills($wars['id']);
